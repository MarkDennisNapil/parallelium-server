...
exports.Search = (req, res) => {
  ...
  Promise.all([
    userModel.aggregate({ text: { $search: `${keyword}` } }),
    postModel.aggregate({ text: { $search: `${keyword}` } }),
    textModel.aggregate({ $text: { $search: `${keyword}` } }),
    videoModel.aggregate({ $text: { $search: `${keyword}` } }),
    vibeModel.aggregate({ $text: { $search: `${keyword}` } })
  ]).then(data => {
